...
//	private EncodingProvider encodingProvider = new EncodingProviderImpl("ISO-8859-1", "GBK");
	//TODO: Creatxr
	private EncodingProvider encodingProvider = new PlainEncodingProviderImpl();
	...
	protected int dehydrate(Serializable id, Object[] fields, boolean[] includeProperty, PreparedStatement st, SessionImplementor session) throws SQLException, HibernateException {
		...
		//TODO: Creatxr
		Object[] encodedFields = new Object[fields.length];
		for (int i=0; i<encodedFields.length; i++) {
			encodedFields[i] = encodingProvider.convertToDatabaseEncoding(fields[i]);
		}
		...
		for (int j=0; j<getHydrateSpan(); j++) {
			if ( includeProperty[j] ) {
				getPropertyTypes()[j].nullSafeSet( st, encodedFields[j], index, session );
	...
	throws HibernateException {
		...
		try {
			Object retval = ( (UniqueEntityLoader) loaders.get(lockMode) ).load(session, id, optionalObject);
			//TODO: Creatxr
			retval = encodingProvider.convertToApplicationEncoding(retval);
			return  retval;
		}
	...
	public EntityPersister(PersistentClass model, SessionFactoryImplementor factory) throws HibernateException {
		...
		//TODO: set encoding provider
		if (factory.getEncodingProvider()!=null) {
			encodingProvider = factory.getEncodingProvider();
		}
	...
	public void setEncodingProvider(EncodingProvider encoderProvider) {
		this.encodingProvider = encoderProvider;
	}
