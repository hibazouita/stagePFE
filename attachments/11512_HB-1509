...
public abstract class LazyInitializer
{
	...
	public void initialize() throws HibernateException
	{
		if (target == null)
		{
			...
			{
				// begin patched TZX hibernate code
				org.apache.commons.logging.Log log =
					LogFactory.getLog(LazyInitializer.class);
				if (log.isDebugEnabled())
				{
					log.debug(
						"LazyInitializer is dynamically reacquiring Session DB connection.");
				}
				try
				{
					session.reconnect();
				...
				catch (Exception e)
				{
					throw new HibernateException(
						"Could not initialize proxy - the owning Session could not be reconnected",
						e);
				}
				...
				finally
				{
					...
					{
						log.debug(
							"LazyInitializer is dynamically released Session DB connection.");
					}
				}

				//throw new HibernateException("Could not initialize proxy - the owning Session is disconnected");
				// end patched TZX hibernate code
			}
			else
			{
				target = session.immediateLoad(persistentClass, id);
	...
	private void initializeWrapExceptions()
	{
		...
		{
			LogFactory.getLog(LazyInitializer.class).error(
				...
				if (target == null && session != null)
					target =
						session.getEntity(
							new Key(
								id,
								session.getFactory().getPersister(
									persistentClass)));
				if (target == null)
				{
					if (replacement == null)
